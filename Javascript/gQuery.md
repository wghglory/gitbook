...
jquery 原理 源码 核心架构 事件委托 插件机制：<http://www.zhangxinxu.com/wordpress/2013/07/jquery-%E5%8E%9F%E7%90%86-%E6%9C%BA%E5%88%B6/>
...
$$.prototype = {
  ...
  arrayExtend: function() {
    ...
    Array.prototype.indexOf = function(target, start) {
      ...
      if (start < 0) start = 0; //如果指定的搜索位置小于0，则设置其开始搜索位置为0
      for (; start < l; ++start) {
        if (this[start] === target) return start;
      }
      return -1;
    };
    ...
    Array.prototype.lastIndexOf = function(target, start) {
      var l = this.length;
      if (start === void 0) start = this.length;
  ...
  // = {     "name": "guanghui" }; console.log(formatString("欢迎@(name)来到百度世界",
  // user));
