...
ClassName.prototype = {
    ...
    setModalPosition: function(dom) {
        ...
        dom.indicatorModal.style.left = indicatorPosition.left - dom.indicatorModal.offsetWidth + dom.indicator.offsetWidth + 'px';
    ...
    bindIndicatorModalDOM: function(data) {
        ...
        for (var i = 0; i < data.body.length; i++) {
            ...
            strBody +=
                '<tr><td>' + data.body[i].product + '</td><td><span class="activity ' + activity.className + '" data-diff="' + activity.diff + '"></span></td><td>' + data.body[i].lastActiveDate + '</td></tr>';
        }
    ...
    bindEvents: function() {
        ...
        var instance = this;
        var dom = this.doms;
        dom.indicator.onmouseenter = function() { //hold on .5s to display
        ...
        dom.indicatorModal.onmouseenter = function() { //fast move from icon to modal window, keep modal open
