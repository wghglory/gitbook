...
1.  高效：核心设计基于 Unix 流的概念。通过利用 Node.js 强大的**流，通过管道连接，不需要往磁盘写中间文件**，可以更快地完成构建。
1.  **插件纯粹**：Gulp 的每个插件只完成一个功能，这也是 Unix 的设计原则之一，各个功能通过流进行整合并完成复杂的任务。
1.  Gulp 的核心 API 只有 5 个
