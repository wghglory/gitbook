...
服务器设置 `Access-Control-Allow-Origin` HTTP 响应头之后，允许浏览器跨域请求。浏览器在头信息之中，增加一个 Origin 字段。Origin 字段用来说明，本次请求来自哪个源（协议 + 域名 + 端口）。服务器根据这个值，决定是否同意这次请求。CORS 支持所有类型的 HTTP 请求
